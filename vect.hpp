...
struct vect {
    ...
    constexpr inline const d& y() const noexcept {return data[1];}

    vect<d,2> xx() const {return {std::array{data[0],data[0]}};}
    vect<d,2> yy() const {return {std::array{data[1],data[1]}};}
    vect<d,2> yx() const {return {std::array{data[1],data[0]}};}
    vect<d,2> xy() const {return {std::array{data[0],data[1]}};}
    vect4<d> xyyx() const {return {data[0],data[1],data[1],data[0]};}
...
template <typename d>\
constexpr inline vect##size<d> abs(const vect<d,size>& obj) {\
    vect<d,size> outp;\
    _Pragma("GCC ivdep")\
    for (int i{0}; i < size; ++i) {\
        outp.data[i] = std::abs(obj.data[i]);\
    }\
    return outp;\
}
////////////////////////////////////////////////////////////////////////////////////////////////////////
...
struct vect3 : vect<d,3>{
    ...
    constexpr inline d& z() noexcept {return this->data[2];}
    constexpr inline const d& z() const noexcept {return this->data[2];}
};
////////////////////////////////////////////////////////////////////////////////////////////////////////
mfuncvect(3);
...
struct vect4 : vect<d, 4>{
    ...
    constexpr inline const d& h() const noexcept {return this->data[3];}
};
////////////////////////////////////////////////////////////////////////////////////////////////////////
mfuncvect(4);
